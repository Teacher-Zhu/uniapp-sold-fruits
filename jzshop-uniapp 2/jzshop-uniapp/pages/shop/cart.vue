<template>
	<view class="wrap">
						...
						<view class="gl-price">
							<u-row>
								...
								<u-col :span="6" v-if="rightText=='编辑'">
									<u-number-box v-model="item.count" :min="1" :max="item.goods.stock"
										@change="countChange(item)"></u-number-box>
								</u-col>
							</u-row>
						</view>
		...
		<view class="navigation" v-if="isLogin &&cartList.length>0">
			...
			<view class="right">
				<u-button type="warning" v-if="rightText =='完成'" @click="addFav" style="margin-right:2rpx;">移入收藏
				</u-button>
	...
	export default {
		...
		methods: {
			init() {
				...
				if (this.isLogin) {
					// uni.request({
					// 	url: baseApi + '/user/shopping/query',
					// 	method: 'POST',
					// 	headers: {
