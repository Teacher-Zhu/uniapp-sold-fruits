					...
					<view class="sku-list">
						<block v-for="(category,index) in sku.tree" :key="index">
							...
							<view class="node-list">
								<u-tag class="node" :type="node.mode=='disable'?'info':'warning'"
									v-for="(node,index2) in category.v" :key="index2" :text="node.name"
									:mode="node.mode=='default'?'light':(node.mode=='select'?'dark':'light')"
	...
	export default {
		...
		methods: {
			...
			buy() {
				...
				// uni.request({
				// 	url: 'http://localhost:8081/user/cart/add',
				// 	method: 'POST',
				// 	data: params,
				// 	success: (response) => {
				// 		console.log("点击了加入购物车：", response)
				// 		this.showSku = false;
